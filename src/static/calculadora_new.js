document.addEventListener('DOMContentLoaded', function() {
    ...
    const elements = {
        ...
        service: {
            ...
            input: document.getElementById('service-input'),
            total: document.getElementById('service-total'),
            decreaseBtn: document.querySelector('.decrease[data-type="service"]'),
            increaseBtn: document.querySelector('.increase[data-type="service"]')
        },
    ...
    function calculateTotal(type) {
        ...
        // Para conversas de serviço, as primeiras 1000 são gratuitas
        if (type === 'service') {
            const billableQuantity = Math.max(0, quantity - 1000);
            const totalInReais = billableQuantity * rateInDollars * exchangeRate;
            return totalInReais;
        } else {
    ...
    function updateGrandTotal() {
        ...
        const serviceTotal = calculateTotal('service');
        
        const grandTotal = marketingTotal + utilityTotal + authTotal + serviceTotal;
    ...
    Object.keys(elements).forEach(type => {
        if (type !== 'grandTotal' && type !== 'exportBtn') {
    ...
    elements.exportBtn.addEventListener('click', function() {
        ...
        // Serviço
        const serviceQty = parseInt(elements.service.input.value) || 0;
        const serviceTotal = calculateTotal('service');
        doc.text('Serviço', 20, y);
        doc.text(serviceQty.toString(), 100, y);
        doc.text(serviceTotal.toFixed(2).replace('.', ','), 150, y);
